<div class="space-y-8">
    ...
    <x-card class="space-y-6">
        ...
        @elseif ($step === 4)
            <div class="grid gap-4 sm:grid-cols-2">
                <x-input wire:model.live="shippingMethod" name="shipping_method" label="Dostava" placeholder="Brza pošta / lično preuzimanje" />
        ...
        @else
            <x-alert variant="success">Pregled korak je spreman za objavu ili spremanje drafta kada backend validacija bude priključena.</x-alert>
            <div class="grid gap-4 sm:grid-cols-2">
                <x-price-display :amount="$startPrice !== '' ? $startPrice : '1250.00'">Startna cijena</x-price-display>
                <div>
                    <p class="price-label">Trajanje</p>
                    <p class="price">7 dana</p>
                </div>
            </div>
        @endif
