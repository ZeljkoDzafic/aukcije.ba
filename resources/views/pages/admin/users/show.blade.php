        ...
        <div class="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <h1 class="mt-2 text-3xl font-semibold text-slate-900">{{ $userName }}</h1>
                    <p class="mt-2 text-slate-600">{{ $roleSummary }} · KYC nivo {{ $kycLevel }} · {{ ucfirst((string) $tierLabel) }} tier</p>
                </div>
            </div>
            <div class="grid gap-3 sm:grid-cols-2">
                <div class="panel-metric">
                    <p class="text-xs uppercase tracking-[0.24em] text-slate-500">Status</p>
                    <p class="mt-2 text-lg font-semibold {{ $statusClass }}">{{ $statusLabel }}</p>
                </div>
                <div class="panel-metric">
                    <p class="text-xs uppercase tracking-[0.24em] text-slate-500">Trust</p>
                    <p class="mt-2 text-lg font-semibold text-slate-900">{{ $trustScore }} / 5</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid gap-8 xl:grid-cols-[0.9fr_1.1fr]">
        <x-card class="panel-shell space-y-5">
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div class="panel-subtle-card"><p class="text-sm text-slate-500">Email</p><p class="mt-1 font-medium text-slate-900">{{ $email }}</p></div>
                <div class="panel-subtle-card"><p class="text-sm text-slate-500">Telefon</p><p class="mt-1 font-medium text-slate-900">{{ $phone }}</p></div>
                <div class="panel-subtle-card"><p class="text-sm text-slate-500">Status</p><p class="mt-1 font-medium {{ $statusClass }}">{{ $statusLabel }}</p></div>
                <div class="panel-subtle-card"><p class="text-sm text-slate-500">Trust score</p><p class="mt-1 font-medium text-slate-900">{{ $trustScore }} / 5</p></div>
            </div>
        </x-card>
        ...
        <div class="space-y-6">
            ...
            <x-card class="panel-shell space-y-4">
                ...
                <div class="space-y-3">
                    ...
                    @empty
                        <div class="panel-subtle-card text-sm text-slate-600">Nema aktivnih KYC zahtjeva za ovog korisnika.</div>
                    @endforelse
                </div>
            </x-card>
            ...
            <x-card class="panel-shell space-y-4">
                ...
                <div class="space-y-3">
                    ...
                    @empty
                        <div class="panel-subtle-card text-sm text-slate-600">Još nema zabilježenih admin odluka za ovog korisnika.</div>
                    @endforelse
                </div>
            </x-card>
        </div>
